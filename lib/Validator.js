!function() {
	...
	Validator = module.exports = new Class({
        ...
        , _buildRules: function(rules) {
            Object.keys(rules).forEach(function(propertyName) {
                ...
                // subvalidator
                if (rules[propertyName] instanceof Validator) {
                    this._validators[propertyName].push(rules[propertyName]);
                }
                else {
        ...
        , _validate: function(input, index, language) {
            if (type.object(input)) {
                return Promise.all(Object.keys(input).map(function(propertyName) {
                    ...
                    else if (this._validators[propertyName].length === 1 && this._validators[propertyName][0] instanceof Validator) {
                        return this._validators[propertyName][0].validate(input[propertyName]);
                    }
                    else {
        ...
        , _executeValidator: function(value, validator, propertyName, index, language, callback) {
            if (!validator.isAsync) {
                ...
                validator.isValid(value, function(isValid, data) {
                    this._evaluateResult(isValid, value, data, validator, propertyName, index, language, callback);
                }.bind(this));
            }
        }
