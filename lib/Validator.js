                ...
                return Promise.all(Object.keys(input).map(function(propertyName) {
                    if (!this._validators[propertyName]) return Promise.reject(this._fillText(language, '_invalid_property', '', propertyName, index));
                        ...
                        return new Promise(function(resolve, reject) {
                            ...
                            // no parralell work!
                            executor = function() {
                                this._executeValidator(input[propertyName], this._validators[propertyName][validatorIndex], propertyName, index, language, function(isValid, message) {
                                    if (isValid === false) reject(message);
                                    else if (isValid === true && this._validators[propertyName][validatorIndex].stopValidation) resolve();
        ...
        , _executeValidator: function(value, validator, propertyName, index, language, callback) {
            if (!validator.isAsync) {
                if (validator.isValid(value)) callback(true);
                    ...
                    if (validator.isOptional) callback();
