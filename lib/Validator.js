	...
	module.exports = new Class({
        ...
        , _executeValidator: function(value, validator, propertyName, index, language, callback) {
            if (!validator.isAsync) {
                ...
                else callback(false, this._fillText(language, validator.name, validator.name, propertyName, index, validator.getData(value)));
            }
            else {
                validator.validate(value, function(isValid, data) {
                    if (isValid) callback(true);
                    else callback(false, this._fillText(language, validator.name, validator.name, propertyName, index, data));
                }.bind(this));
            }
        }
