	...
	module.exports = new Class({
        ...
        , _buildRules: function(rules) {
            Object.keys(rules).forEach(function(propertyName) {
                ...
                Object.keys(rules[propertyName]).forEach(function(validatorName) {
                    if (!this.collection.validators[validatorName]) throw new Error('Cannot validate proeprty «'+propertyName+'» with the validator «'+validatorName+'». The validator does not exist!');
                ...
                this._validators[propertyName].forEach(function(validatorInstance) {
                    validatorInstance.prepare();

                    // check if the function accepts on or two parameters
                    validatorInstance.isAsync = /function\s*\([^,]+\,[^\)]+/gi.test(validatorInstance.validate.toString());
                }.bind(this));
            }.bind(this));
        }
        ...
        , _validate: function(input, index, language) {
            if (type.object(input)) {
                return Promise.all(Object.keys(input).map(function(propertyName) {
                    ...
                    else if (!this._validators[propertyName].length) return Promise.resolve();
                    else {
                        return new Promise(function(resolve, reject) {
                            var   count = this._validators[propertyName]
                                , validatorIndex = 0
                                , executor;

                            // execute validators in the order they are in the array, 
                            // no parralell work!
                            executor = function(isValid, message) {
                                if (!isValid) reject(message);
                                else if (++validatorIndex === count) resolve();
                                else this._executeValidator(input[propertyName], this._validators[propertyName][validatorIndex-1], propertyName, index, language, executor);                                
                            }.bind(this);
            ...
            else return Promise.reject(this.translations[language]._no_input);
        }   
        ...
        , _executeValidator: function(value, validator, propertyName, index, language, callback) {
            if (!validator.isAsync) {
                if (validator.isValid(value)) callback(true);
                else callback(false, this._fillText(language, validator.name, validator.name, propertyName, index));
            }
            else {
                validator.validate(value, function(isValid) {
         ...
         * fill message text
         */
        , _fillText: function(language, id, validatorName, propertyName, index) {
            var message;

            if (!this.translations[language] || !this.translations[language][id]) {
                if (this.translations.en[id]) message = this.translations.en[id];
                else message = 'Failed to get a validation message for the id «'+id+'»!'
            }
            else message = this.translations[language][id];

            message = message.replace(/\$validator/gi, '«'+validatorName+'»');
            message = message.replace(/\$index/gi, !type.undefined(index) ? 'at index «'+index+'»' : '');
