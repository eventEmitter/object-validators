!function() {
    ...
    ValidatorConfig = module.exports = new Class({
        ...
        /**
         * returns a specific validator
         */
        , getValidator: function(name) {
            return this.validators[name] ? this.validators[name].validator : null;
        }
        ...
        , _loadValidators: function() {
            ...
            files.forEach(function(validatorName) {
                ...
                // remove priority
                validatorName = validatorName.replace(/^[0-9-]+/gi, '').replace('.js', '');
        ...
        , _loadTranslations: function() {
            ...
            files.forEach(function(translationName) {
                ...
                translationName = translationName.replace('.js', '');
    ...
    /**
     * global validator instance that should not be configured
     */
    ValidatorConfig.Validator = function(rules) {
        if (!config) config = new ValidatorConfig();

        return config.createValidator(rules);
    };


    /**
     * let the user get global validators
     */
    ValidatorConfig.Validator.getValidator = function(id) {
        if (!config) config = new ValidatorConfig();

        return config.getValidator(id);
    }
}();
